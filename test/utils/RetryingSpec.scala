...
class RetryingSpec extends VatRegSpec {

  override lazy val app: Application = new GuiceApplicationBuilder()
    .in(Environment.simple(mode = Mode.Dev))
    .configure("metrics.enabled" -> "false")
    .overrides(bind[ActorSystem].toInstance(ActorSystem.apply()))
    .build()
  ...
  "Retrying" when {
    ...
    "initially fails with failed future" must {
      ...
      "retry successfully" in {
        retrying
          ...
          .futureValue mustBe s"Attempt $numRetries"
      }
    ...
    "fails" must {
      "allow aborting without using remaining retries" in {
        ...
        retrying
          ...
          .failed
          .futureValue mustBe timeoutException
      }
    }
    ...
    "initially fails because bad value returned" must {
      ...
      "retry successfully" in {
        retrying
          ...
          .futureValue mustBe s"Ok $numRetries"
      }
    ...
    "getting fibonacci delays" must {
      ...
      "must allow to be scaled by a factor" in {
        Retrying.fibonacciDelays(500.milliseconds, 5) mustBe Seq(
          500.milliseconds,
          500.milliseconds,
          1000.milliseconds,
          1500.milliseconds,
