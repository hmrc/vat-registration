...
class SdesServiceSpec
  ...
  def testCallback(optFailureReason: Option[String], notificationType: String = testNotificationType): SdesCallback =
    SdesCallback(
  ...
  "notifySdes" must {
    "create a payload for every upscanDetails in repository and send it" in {
      ...
      referenceList.zipWithIndex.map { case (reference, index) =>
        mockNotifySdes(
          testPayload(reference, Some(testNrsId), index),
          Future.successful(SdesNotificationSuccess(NO_CONTENT, ""))
        )
      }

      val result = await(TestService.notifySdes(testRegId, testFormBundleId, Some(testNrsId), testProviderId))
      result mustBe Seq(
        SdesNotificationSuccess(NO_CONTENT, ""),
        SdesNotificationSuccess(NO_CONTENT, ""),
        SdesNotificationSuccess(NO_CONTENT, "")
      )
      ...
      eventually {
        verifyAudit(
          SdesFileSubmissionAudit(
            testPayload(testReference, Some(testNrsId), 0),
            SdesNotificationSuccess(NO_CONTENT, ""),
            testProviderId
          )
        )
        verifyAudit(
          SdesFileSubmissionAudit(
            testPayload(testReference2, Some(testNrsId), 1),
            SdesNotificationSuccess(NO_CONTENT, ""),
            testProviderId
          )
        )
        verifyAudit(
          SdesFileSubmissionAudit(
            testPayload(testReference3, Some(testNrsId), 2),
            SdesNotificationSuccess(NO_CONTENT, ""),
            testProviderId
          )
        )
    ...
    "create a payload without nrsSubmissionId and send it" in {
      ...
      eventually {
        verifyAudit(
          SdesFileSubmissionAudit(
            testPayload(testReference, None, 0),
            SdesNotificationSuccess(NO_CONTENT, ""),
            testProviderId
          )
        )
        verifyAudit(
          SdesFileSubmissionAudit(
            testPayload(testReference2, None, 1),
            SdesNotificationSuccess(NO_CONTENT, ""),
            testProviderId
          )
        )
        verifyAudit(
          SdesFileSubmissionAudit(
            testPayload(testReference3, None, 2),
            SdesNotificationSuccess(NO_CONTENT, ""),
            testProviderId
          )
        )
