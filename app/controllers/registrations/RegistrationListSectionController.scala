...
@Singleton
class RegistrationListSectionController @Inject() (
  ...
  def getSectionIndex(regId: String, section: CollectionSectionId, index: Int): Action[AnyContent] = Action.async {
    implicit request =>
  ...
  def replaceSectionIndex(regId: String, section: CollectionSectionId, index: Int): Action[JsValue] =
          ...
          sectionValidationService.validateIndex(section, request.body).flatMap {
            case Right(ValidSection(validatedJson)) =>
              registrationService.upsertSectionIndex(internalId, regId, section, validatedJson, index).map {
                ...
                case _       =>
                  errorLog(
                    s"[RegistrationListSectionController][replaceSectionIndex] errored with Unable to upsert section '${section.key}' for regId '$regId'"
                  )
                  InternalServerError(
                    s"[RegistrationListSectionController] Unable to upsert section '${section.key}' for regId '$regId'"
                  )
              }
            case Left(response @ InvalidSection(_)) =>
  ...
  def deleteSectionIndex(regId: String, section: CollectionSectionId, index: Int): Action[AnyContent] = Action.async {
    implicit request =>
      isAuthenticated { internalId =>
        isValidIndex(section, index) {
