...
case class SubmissionAuditModel(
  userAnswers: JsValue,
  vatScheme: VatScheme,
    ...
    (vatScheme.eligibilitySubmissionData, vatScheme.applicantDetails, vatScheme.vatApplication) match {
      case (Some(eligibilityData), Some(applicantDetails), Some(vatApplication)) =>
        jsonObject(
          ...
          "registrationReason"    -> {
            if (vatApplication.currentlyTrading.contains(false) && eligibilityData.registrationReason.equals(Voluntary))
              IntendingTrader.humanReadableKey
          ...
          "idsVerificationStatus" -> applicantDetails.entity.map(entity =>
            entity.bpSafeId.fold(
