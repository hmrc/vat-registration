    ...
    registrationRepository.retrieveVatScheme(regId).flatMap {
      case Some(vatScheme) =>
        oldEligibilityData match {
          case EligibilitySubmissionData(_, _, _, _, oldPartyType, _, _, _) if !oldPartyType.equals(eligibilityData.partyType) =>
          ...
          case EligibilitySubmissionData(_, _, oldTurnoverEstimates, _, _, _, _, oldTransactorFlag)
