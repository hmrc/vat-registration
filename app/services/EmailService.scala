...
@Singleton
class EmailService @Inject() (emailConnector: EmailConnector, registrationMongoRepository: VatSchemeRepository)(implicit
  ex: ExecutionContext
  ...
  private def missingDataLog(missingItem: String, regId: String) =
    s"Unable to send submission received email for $regId due to missing $missingItem"

  // scalastyle:off
  def sendRegistrationReceivedEmail(internalId: String, regId: String, lang: String)(implicit
    hc: HeaderCarrier,
    request: Request[_]
    ...
    (for {
      ...
      vatScheme     = optVatScheme.getOrElse {
                        errorLog(
                          "[EmailServiceEmailService][sendRegistrationReceivedEmail][submissionParameters] - VAT scheme"
                        )
    ...
    } yield response).recover { case _: Exception =>
      EmailFailedToSend
    }
  }
