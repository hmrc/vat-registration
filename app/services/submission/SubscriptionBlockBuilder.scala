...
class SubscriptionBlockBuilder @Inject() () extends LoggingUtils {
  ...
  def buildSubscriptionBlock(vatScheme: VatScheme)(implicit request: Request[_]): JsObject =
    (
      ...
      vatScheme.applicantDetails,
      vatScheme.business,
      vatScheme.otherBusinessInvolvements.getOrElse(Nil)
    ) match {
      case (
            Some(eligibilityData),
            Some(vatApplication),
            Some(applicantDetails),
            Some(business),
            otherBusinessInvolvements
          ) =>
        jsonObject(
          "reasonForSubscription" -> jsonObject(
            "registrationReason"   -> {
              if (
                vatApplication.currentlyTrading.contains(false) && eligibilityData.registrationReason.equals(Voluntary)
              ) IntendingTrader.key
          ...
          optional("schemes"                 -> vatScheme.flatRateScheme.flatMap { flatRateScheme =>
            (
              flatRateScheme.joinFrs,
              flatRateScheme.categoryOfBusiness,
              flatRateScheme.percent,
              flatRateScheme.frsStart,
              flatRateScheme.limitedCostTrader
            ) match {
              ...
              case _                                                                                               =>
                errorLog(
                  "[SubscriptionBlockBuilder][buildSubscriptionBlock] - FRS scheme data missing when joinFrs is true"
                )
                throw new InternalServerException(
                  "[SubscriptionBlockBuilder] FRS scheme data missing when joinFrs is true"
                )
            }
          }),
            ...
            jsonObject(
              ...
              "acceptTsAndCsForTOGCOrCOLE" -> (if (togcData.wantToKeepVatNumber) {
                                                 togcData.agreedWithTermsForKeepingVat.getOrElse {
                                                   errorLog(
                                                     "[SubscriptionBlockBuilder][buildSubscriptionBlock] - TOGC user wants to keep VRN but did not answer T&C"
                                                   )
                                                   throw new InternalServerException(
                                                     "TOGC user wants to keep VRN but did not answer T&C"
                                                   )
                                                 }
                                               } else {
                                                 false
      ...
      case _ =>
        errorLog(
          "[SubscriptionBlockBuilder][buildSubscriptionBlock] - Could not build subscription block for submission because some of the data is missing"
        )
        throw new InternalServerException(
