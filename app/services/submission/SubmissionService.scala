...
import play.api.http.Status.{BAD_REQUEST, CONFLICT, UNAUTHORIZED}
...
class SubmissionService @Inject() (
  ...
  def submitVatRegistration(internalId: String, regId: String, userHeaders: Map[String, String], lang: String)(implicit
    ...
    (for {
      ...
      optFormBundle      <- handleResponse(submissionResponse, submission, regId, correlationId, internalId)
  ...
  private def notifyNrs(registrationId: String, nrSubmissionId: String, correlationId: String)(implicit
      request: Request[_],
      hc: HeaderCarrier): Future[Seq[Unit]] =
      ...
      upscanDetailsList.map { upscanDetails =>
        ...
        if (url.isDefined && optChecksum.isDefined && optMimeType.isDefined) {
          ...
          if (isEnabled(PostSubmissionDecouplingConnector)) {
            ...
            infoLog(
              s"[SubmissionService][notifyNrs] Not sending NRS attachment as PostSubmissionDecouplingConnector is off. Attachment $attachmentId")
          }
        } else {
          pagerduty(
  ...
  private[services] def submit(submission: Either[BuildFailure, JsObject], regId: String, correlationId: String)(implicit
    ...
    submission match {
      case Left(buildFailure) =>
        ...
        errorLog(
          s"[SubmissionService][submit] Failed to submit as submission body was unable to be built, for reason: ${buildFailure.reason}." +
            s"\nReturning 400",
          regId
        )
      ...
      case Right(submissionModel) =>
        authorised().retrieve(credentials) {
          ...
          case None =>
            errorLog(s"[SubmissionService][submit] Unable to fetch auth credentials for user with registration ID: $regId", regId)
            Future.successful(Left(VatSubmissionFailure(UNAUTHORIZED, s"Unable to fetch auth credentials for user with registration ID: $regId")))
        }
    }
  }
  ...
  private[services] def handleResponse(
      ...
      submissionPayload: Either[BuildFailure, JsObject],
      regId: String,
      correlationId: String,
      internalId: String
    ...
    (submissionPayload, vatSubmissionResponse) match {
      case (Left(buildFailure), _) =>
        errorLog(s"[SubmissionService][handleResponse] Submission failed due to invalid submission body: ${buildFailure.reason}", regId)
        registrationRepository
          .updateSubmissionStatus(internalId, regId, VatRegStatus.failed)
          .map(_ => None)
      case (Right(_), Left(VatSubmissionFailure(CONFLICT, _))) =>
      ...
      case (Right(submissionBody), Left(VatSubmissionFailure(BAD_REQUEST, _))) =>
        ...
        for {
          ...
          errors = schemaValidationService.validate(apiSchema, submissionBody.toString())
      ...
      case (Right(_), Left(VatSubmissionFailure(status, reason))) =>
        ...
        registrationRepository
          ...
          .map(_ => None)
      case (Right(_), Right(VatSubmissionSuccess(formBundleId))) =>
  ...
  private[services] def logSubmission(vatScheme: VatScheme, vatSubmissionStatus: VatSubmissionResponse)(implicit
    ...
    val regReason =
      if (vatScheme.vatApplication.exists(_.currentlyTrading.contains(false)) &&
