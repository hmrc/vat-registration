    ...
    (vatScheme.eligibilitySubmissionData, vatScheme.vatApplication) match {
      case (Some(eligibilityData), Some(vatApplication)) =>
        jsonObject(
          ...
          "attachments"           -> (jsonObject(optional("EORIrequested" -> vatApplication.eoriRequested)) ++ {
      ...
      case (None, Some(_))                               =>
        errorLog(
          "[AdminBlockBuilder][buildAdminBlock] - Could not build admin block for submission due to missing eligibility data"
        )
        throw new InternalServerException("Could not build admin block for submission due to missing eligibility data")
      case (Some(_), None)                               =>
        errorLog(
          "[AdminBlockBuilder][buildAdminBlock] - Could not build admin block for submission due to missing vat application data"
        )
        throw new InternalServerException(
          "Could not build admin block for submission due to missing vat application data"
        )
      case _                                             =>
        errorLog(
          "[AdminBlockBuilder][buildAdminBlock] - Could not build admin block for submission due to no available data"
        )
